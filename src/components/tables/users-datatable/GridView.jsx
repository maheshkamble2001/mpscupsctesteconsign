...
function Item({ row, table }) {
  ...
  // Fix: Provide default option if not found
  const option = rolesOptions.find((item) => item.value === row.original.role) || {
    color: "primary",
    label: row.original.role || "N/A"
  };
    ...
    >
      ...
      <div className="mx-auto mt-4 inline-grid grid-cols-1 gap-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <p className="truncate"> {row.original.mobile || row.original.phone || "-"}</p>
        </div>
        <div className="flex min-w-0 items-center gap-2">
          <div className="flex size-6 items-center justify-center rounded-lg bg-primary-600/10 text-primary-600 dark:bg-primary-400/10 dark:text-primary-400">
    ...
    <>
      <div className="flex justify-center gap-1 py-2">
        <Button 
          onClick={() => {
            // Handle view profile
            if (table.options.meta?.onViewUser) {
              table.options.meta.onViewUser(row.original.user_id);
            }
          }}
          className="h-7 space-x-1.5 rounded-full px-3 text-xs"
        >
          ...
          >
            ...
            <MenuItem>
              {({ focus }) => (
                <button
                  onClick={() => {
                    if (table.options.meta?.onEditUser) {
                      table.options.meta.onEditUser(row.original.user_id);
                    }
                  }}
                  className={clsx(
                    "flex h-9 w-full items-center gap-3 px-3 tracking-wide outline-hidden transition-colors",

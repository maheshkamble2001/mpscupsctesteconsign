...
// API Layer Imports
import { addTest, updateTest } from "api/applicationmanagement/tests";
...
export default function AddTest() {
    ...
    const state = location.state?.testData || {};
    ...
    const loadTestDetails = async () => {
        try {
            ...
            if (!data || Object.keys(data).length === 0) {
                console.log("No test data to load");
                return;
            }

            // ✅ Set simple fields
            setValue("TestName", data.TestName || "");
            setValue("TestType", data.TestType || "Regular");
            setValue("ExamId", data.ExamId ? String(data.ExamId) : "");
            setValue("Duration", data.Duration || "");
            setValue("Description", data.Description || "");
            setValue("Tags", data.Tags || "");
            setValue("noOfQuestions", data.noOfQuestions || 0);
            setValue("marksPerQuestion", data.marksPerQuestion || 0);
            setValue("negativeMarks", data.negativeMarks || 0);
            setValue("Attempts", data.Attempts || 0);

            // ✅ Boolean flags
            setValue("isShuffle", data.isShuffle || false);
            setValue("isAnswerShuffle", data.isAnswerShuffle || false);
            setValue("isAllowReview", data.isAllowReview !== undefined ? data.isAllowReview : true);
            setValue("isShowWarningTimer", data.isShowWarningTimer !== undefined ? data.isShowWarningTimer : true);
            setValue("isDisableRightClick", data.isDisableRightClick || false);

            // ✅ Languages (string → array)
            if (data.languages) {
                const languagesArray = Array.isArray(data.languages) ? data.languages : data.languages.split(",");
            ...
            if (data.subjects && Array.isArray(data.subjects) && data.subjects.length > 0) {
                const subjects = data.subjects.map(s => ({
                    ...
                    questionCount: String(s.questionCount)
                }));
                replace(subjects);
            }
            ...
            console.error("Error loading test details:", e);
    ...
    // Fix: Track rows to determine selected values in real-time
    const watchedSubjects = watch("subjects") || [];

    // Fix: Auto-calculate total questions - removed isQuestionCountManuallyEdited dependency
    useEffect(() => {
    ...
    const handleExamChange = async (selectedExamId, hookFormOnChange) => {
        ...
        try {
            ...
            if (examRes?.code === 200 && examRes?.data) {
                ...
                // Only set these if not in edit mode with existing data
                if (!isDataLoaded) {
                ...
                if (Array.isArray(examBlueprint.Subjects) && examBlueprint.Subjects.length > 0) {
                    ...
                    // Only create new structures if not in edit mode or if subjects are empty
                    const currentSubjects = getValues("subjects");
                    if (!isDataLoaded || (currentSubjects.length === 1 && !currentSubjects[0].SubjectId)) {
    ...
    const handleSubjectChange = (index, selectedItem, hookFormOnChange) => {
    ...
    const handleUpdateTest = async (data) => {
        try {
            ...
            // Fix: Handle missing access_token
            if (!access_token) {
            ...
            const payload = {
                TestId: state.TestId, // Ensure TestId is included for updates
            ...
            toast.error(err?.message || "An error occurred during transaction deployment execution");
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                        <div className="px-6 py-5 sm:px-8 sm:py-6 border-b border-gray-200 bg-gray-50">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Update Test</h2>
                                </div>
                            </div>
                        </div>

                        <form onSubmit={handleSubmit(handleUpdateTest)} className="p-6 sm:p-8 space-y-8">
                                ...
                                <div className="grid gap-5 sm:grid-cols-2">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        {errors?.TestType?.message && (
                                            <p className="text-xs text-red-500 mt-1">{errors.TestType.message}</p>
                                        )}
                                    </div>
                                                    ...
                                                    <Controller
                                                        ...
                                                        render={({ field: { value, onChange } }) => (
                                                            <Combobox
                                                                ...
                                                                onChange={(val) => handleSubjectChange(index, val, onChange)}
                                                                displayField="label"
                                                                searchFields={["label"]}

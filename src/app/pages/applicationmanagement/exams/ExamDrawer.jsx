...
export function ExamDrawer({ isOpen, close, exam, onToggleSettings }) {
  ...
  // Naya Colorful InfoRow Component
  ...
  const Section = ({ title, children }) => (
    <div className="mb-8">
      <h3 className="mb-3.5 px-2 text-[11px] font-bold tracking-widest text-slate-400 uppercase">
        {title}
      </h3>
      <div className="overflow-hidden rounded-[20px] border border-slate-200/60 bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] divide-y divide-slate-100">
        {children}
      </div>
    </div>
  );
              ...
              >
                <DialogPanel className="pointer-events-auto w-screen max-w-[420px]">
                  <div className="flex h-full flex-col bg-[#F8FAFC] shadow-2xl">
                    ...
                    {/* Header */}
                    <div className="flex items-center justify-between border-b border-slate-200/60 bg-white px-7 py-6 shadow-sm z-10">
                      <div className="flex items-center gap-4">
                        <div className="flex h-[46px] w-[46px] shrink-0 items-center justify-center rounded-[14px] bg-[#0A0E17] text-[18px] font-bold text-white shadow-md uppercase">
                        ...
                        <div className="flex flex-col">
                          ...
                          <div className="mt-1 flex items-center gap-2">
                            ...
                            {e.Stage && (
                              <span className="inline-flex items-center rounded-full bg-orange-50 px-2.5 py-0.5 text-[10px] font-bold text-orange-600 uppercase tracking-widest">
                                {e.Stage}
                              </span>
                            )}
                          </div>
                      ...
                      <button
                        onClick={close}
                        className="rounded-full bg-slate-50 p-2 text-slate-400 border border-slate-100 transition-all hover:bg-slate-100 hover:text-slate-700 hover:rotate-90"
                      >
